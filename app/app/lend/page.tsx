...
export default function LendPage() {
  ...
  const lenderState = useLenderState(wallet.publicKey);
  const live = livePool.data;
  const liveLender = lenderState.data;
  const displayPool = useMemo(
    () => ({
      ...
      kaminoAllocationBps:
        ...
        (pool.totalDepositsUsdt > 0
          ? Math.round((yieldAllocation.kaminoUsdt / pool.totalDepositsUsdt) * 10_000)
          : 0),
  ...
  async function runAction(action: "deposit" | "withdraw" | "rebalance") {
    ...
    try {
      ...
      await Promise.all([livePool.refresh(), lenderState.refresh()]);
      setActionState({
        status: "success",
    ...
    <div className="space-y-6">
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
            Supply devnet USDC, track borrower demand, and route idle liquidity
            through the configured Kamino vault under AXIOM policy limits.
          </p>
        </div>
        <Card>
          <CardHeader>
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2">
            <InfoPanel
              ...
              value={
                wallet.connected
                  ? `$${(liveLender?.walletUsdc ?? 0).toLocaleString()}`
                  : "Connect wallet"
              }
            />
            <InfoPanel
              ...
              label="Supplied"
              value={`$${(liveLender?.suppliedUsdc ?? 0).toLocaleString()}`}
            />
          </CardContent>
        </Card>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <StatusState
          message={
            !wallet.connected
              ? "Connect a wallet to load its live lender position."
              : lenderState.error ??
                (liveLender?.lenderPosition
                  ? `Position ${liveLender.lenderPosition} loaded from devnet.`
                  : "No lender position exists for this wallet yet. Deposit to create one.")
          }
          state={
            !wallet.connected
              ? "empty"
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <Card>
          <CardHeader>
            <CardTitle>Yield inputs</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <InfoPanel
              icon={<Coins className="h-5 w-5" />}
              label="Borrower base rate"
              value={`${(displayPool.baseInterestBps / 100).toFixed(2)}%`}
            />
            <InfoPanel
              icon={<RefreshCw className="h-5 w-5" />}
            ...
            />
            <p className="text-sm text-muted-foreground">
              Realized APY and reward totals are hidden until a live rewards
              indexer is connected.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Lender accounting</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <InfoPanel
              icon={<ArrowDownToLine className="h-5 w-5" />}
              label="Deposited"
              value={`$${(liveLender?.depositedUsdc ?? 0).toLocaleString()}`}
            />
            <InfoPanel
              icon={<ArrowUpFromLine className="h-5 w-5" />}
              label="Withdrawn"
              value={`$${(liveLender?.withdrawnUsdc ?? 0).toLocaleString()}`}
            />
            <p className="text-sm text-muted-foreground">
              Last position update: {liveLender?.lastUpdated ?? "No position yet"}
            </p>
          </CardContent>
        </Card>
      </section>

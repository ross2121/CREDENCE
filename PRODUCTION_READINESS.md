...
- Ika CPI: final Ika Solana program ID, instruction interface, account layout, signer rules, and devnet/mainnet deployed addresses. Until then, use Privy for live Solana devnet wallet policy.
...
npm run integration:privy:test
npm run integration:birdeye:test
```

`anchor test --skip-local-validator --skip-deploy` runs against the configured devnet provider and requires reachable devnet RPC.
